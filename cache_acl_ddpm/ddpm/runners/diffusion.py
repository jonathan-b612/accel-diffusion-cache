...
class Diffusion(object):
        ...
        for epoch in tqdm.tqdm(range(start_epoch, self.config.training.n_epochs)):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.accelerator.is_main_process and (step % self.config.training.snapshot_freq == 0 or step == 1) :
                    ...
                    torch.save(
                        states,
                        os.path.join(logger.checkpoint_path, "ckpt_{}.pth".format(step)),
                    )
                    torch.save(states, os.path.join(logger.checkpoint_path, "ckpt.pth"))
    ...
    def creat_model(self):          
        ...
        model.eval()
        return model
    def obtain_generator_para(self):
        try:
            skip = self.args.skip
        except Exception:
            skip = 1
        ...
        if self.args.sample_type == "generalized":
            ...
            generator_function = (generalized_steps)
            ...
            generator_function = (ddpm_steps)
        else:
            raise NotImplementedError
        return seq, generator_function
    ...
    def sample(self):
        ...
        if not self.args.use_pretrained:
            if getattr(self.config.sampling, "ckpt_id", None) is None:
                states = torch.load(
                    os.path.join(logger.checkpoint_path, "ckpt.pth"),
                    map_location='cpu',
                )
            else:
                states = torch.load(
    ...
    def sample_fid(self, model, total_n_samples=50000, save_images = True):
        ...
        n_rounds = (total_n_samples - img_id) // self.args.batch_samples
    ...
    def test(self, model, total_n_samples, timesteps, save_id = None):
        ...
        if self.accelerator.is_main_process:
            ...
            if save_id is not None:
                tvu.save_image(
                    all_samples[:100], os.path.join(logger.image_ckpt_path, f"sample_{save_id}.png")
                )
