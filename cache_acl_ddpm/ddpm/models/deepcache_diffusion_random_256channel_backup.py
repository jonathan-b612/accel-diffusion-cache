...
class Model(nn.Module):
    ...
    def forward(self, x, t, context, **kwargs):  # , prv_f=None, branch=None
        ...
        if prv_f is not None:
            ...
            # 上采样阶段
            # 特殊处理：对于up2.block2缓存位置，需要先经过up2.upsample
            if self.current_cache_description == "up2.block2":
                # 先执行up2.upsample
                # h = self.up[2].upsample(prv_f)
                ...
                if kwargs['prv_f'] is not None:
                    ...
                    if self.a_list is not None:
                        ...
                        h = a * h + b
                h = self.up[2].upsample(prv_f)
                # 获取down1.block1特征进行concat
                if hs:
                    hs_last = hs.pop()  # 弹出down1.block1特征 [5]

                # concat并执行up1.block0
                h = torch.cat([h, hs_last], dim=1)
                h = self.up[1].block[0](h, temb)

                # 继续正常的上采样流程，从up1.block1开始
                for i_level in reversed(range(self.num_resolutions)):
                    if i_level > 1:  # 跳过级别2和3
                        continue

                    for i_block in range(self.num_res_blocks + 1):
                        # 跳过up1.block0（已经执行过）
                        if i_level == 1 and i_block < 1:
                            continue
