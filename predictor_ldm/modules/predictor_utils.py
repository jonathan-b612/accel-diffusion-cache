...
def get_timesteps_for_ddim_quad(length, max_timesteps=1000):
    """
    生成DDIM quadratic时间步序列
    使用公式: seq = np.linspace(0, np.sqrt(max_timesteps * 0.8), length) ** 2

    Args:
        length: 需要的时间步数量 (如100)
        max_timesteps: 总时间步数 (1000)

    Returns:
        timesteps: torch.Tensor, shape [length]
    """
    # 使用numpy生成quadratic序列
    seq = np.linspace(0, np.sqrt(max_timesteps * 0.8), length) ** 2
    seq = [int(s) for s in list(seq)]

    # 转换为torch tensor
    timesteps = torch.tensor(seq, dtype=torch.float32)

    return timesteps

def get_timesteps_for_ddim_uniform( length, num_ddpm_timesteps=1000, verbose=False):
    c = num_ddpm_timesteps // length
    ...
    timesteps = torch.tensor(steps_out, dtype=torch.float32)
    return timesteps