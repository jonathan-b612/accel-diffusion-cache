...
class model_schedule:
    ...
    def correct_cost(self, tolerance, max_init_time, budget):
        ...
        while(1):
            print("================================",self.get_cost())
            if self.get_cost()>(2-tolerance)*budget:
...
class ddim_model_schedule(model_schedule):
    ...
    def random_init_ms(self, tolerance=0.9, max_init_time=200, budget=None):
        ...
        # 1. 确定缓存点位置 - 随机生成时间步
        other_cache_positions = random.sample(range(1, 249), 12)
        cache_positions = [0] + sorted(other_cache_positions)

        # 2. 为每个段随机生成深度（1-4）
        depths = [random.randint(1, 4) for _ in range(13)]


        # 3. 生成完整序列（长度250）
        ms = np.zeros(250, dtype=int)
        ...
        self.ms = ms
        self.cache_index = [(cache_positions[i], depths[i]) for i in range(13)]
...
class controller:
    ...
    def search(self, budget, save_path,seed):
        ...
        population_file = f"../imagenet_best_population/final_population_budget{budget}_seed{seed}.pth"
