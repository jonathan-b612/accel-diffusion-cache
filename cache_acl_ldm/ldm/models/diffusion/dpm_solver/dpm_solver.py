...
class DPM_Solver_trainer:
    def __init__(self, model_fn, quant_model_fn, lr_scheduler, optimizer, noise_schedule, predict_x0=False, thresholding=False, max_val=1.):
        """Construct a DPM-Solver. 
        ...
        """
        self.model = model_fn
        self.quant_model = quant_model_fn
        self.noise_schedule = noise_schedule
        self.predict_x0 = predict_x0
        self.thresholding = thresholding
        self.max_val = max_val
    ...
    def noise_prediction_fn(self, x, t, quant=False):
        """
        Return the noise prediction model.
        """
        if quant:
            return self.quant_model(x, t)
        else:
            return self.model(x, t)


    def data_prediction_fn(self, x, t, quant=False):
        """
        Return the data prediction model (with thresholding).
        """
        noise = self.noise_prediction_fn(x, t, quant=quant)
        dims = x.dim()
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t)
        x0 = (x - expand_dims(sigma_t, dims) * noise) / expand_dims(alpha_t, dims)
        if self.thresholding:
    ...
    def model_fn(self, x, t):
        ...
        if self.predict_x0:
            ...
            return self.noise_prediction_fn(x, t)
    def quant_model_fn(self, x, t):
        """
        Convert the model to the noise prediction model or the data prediction model. 
        """
        if self.predict_x0:
            return self.data_prediction_fn(x, t, quant=True)
        else:
            return self.noise_prediction_fn(x, t, quant=True)
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        ...
        if method == 'adaptive':
            ...
            out = self.model_fn(x, vec_t)
            out_q = self.quant_model_fn(x, vec_t)
            loss = F.mse_loss(out_q, out, size_average=False)
            # print(loss)
            loss.backward()
            self.optimizer.step()
            self.lr_scheduler.step()

            model_prev_list = [out.detach()]
            t_prev_list = [vec_t]
            # Init the first `order` values by lower order multistep DPM-Solver.
            for init_order in range(1, order):
                vec_t = timesteps[init_order].expand(x.shape[0])
                ...
                model_prev_list.append(out.detach())
                t_prev_list.append(vec_t)
            # Compute the remaining values by `order`-th order multistep DPM-Solver.
            for step in range(order, steps + 1):
                vec_t = timesteps[step].expand(x.shape[0])
