...
def log_print(*args, **kwargs):
    ...
    if file_arg is None:
        ...
        # 先打印到控制台
        print(*args, **{k: v for k, v in kwargs.items() if k != 'file'})
        # 然后打印到原始目标
        print(*args, **kwargs)
...
def count_ops_and_params(model, example_inputs, layer_wise=False):
    ...
    for m_name, m in model.named_modules():
        ...
        if layer_wise:
            space = 30 - len(m_name)
            log_print("Layer {}: {} MACs = {:.4f} G, Params = {:.4f} M, MACs% = {:.2f}".format(
                m_name, ' ' * space, layer_flops[m_name] / 1e9, layer_params[m_name] / 1e6,
                        100 * layer_flops[m_name] / flops_count
            ))
    ...
    def add_flops_counter_hook_function(module, ost, verbose, ignore_list):
        if type(module) in ignore_list:
            ...
            if verbose and not type(module) in (nn.Sequential, nn.ModuleList) and \
                ...
                log_print('Warning: module ' + type(module).__name__ +
...
def batch_counter_hook(module, input, output):
    ...
    if len(input) > 0:
        ...
        pass
        log_print('Warning! No positional inputs found for a module,'
...
def add_flops_counter_variable_or_reset(module):
    if is_supported_instance(module):
        if hasattr(module, '__flops__') or hasattr(module, '__params__'):
            log_print('Warning: variables __flops__ or __params__ are already '
